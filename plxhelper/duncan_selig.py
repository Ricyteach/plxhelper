...
def build_Ms_dataframe():
    ...
    for soil_type in soil_types:
        for Ms_tsv_file in Ms_dataframes_path.glob(f'{soil_type}*.tsv'):
            ...
            df_list.append(pd.read_csv(Ms_tsv_file, header=0, names=[Ms_tsv_file.stem], delimiter="\t", index_col=0))  # np.genfromtxt(Ms_tsv_file, delimiter="\t", names=True)
...
def build_duncan_selig_dataframe():
    ...
    df = pd.read_csv(hardening_soil_duncan_selig_path, delimiter="\t", index_col=[0,1])  # np.genfromtxt(Ms_tsv_file, delimiter="\t", names=True)
...
DUNCAN_SELIG_DATAFRAME = DUNCAN_SELIG_INTERPOLATION_DATAFRAME.set_index("Identification", drop=False)
...
def build_hardening_soil_parameters(duncan_selig_soil_type, Ms_psi, σ_z_psi):
    ...
    Ms_with_density_series = pd.Series(Ms_soil_type_series.index.str[2:].astype(float), index=Ms_soil_type_series, name="Density")
    ...
    interpolation_dataframe = DUNCAN_SELIG_INTERPOLATION_DATAFRAME.loc[duncan_selig_soil_type]
    interpolation_dataframe.loc[ρ_rel] = interpolation_dataframe.iloc[0]  # copy non-numerical values (will replace numerical)
    interpolation_dataframe.at[ρ_rel, "Identification"] = duncan_selig_soil_type + f"{ρ_rel:.1f}"  # update model name
    # return duncan_selig_soil_type_dataframe.loc[ρ_rel, duncan_selig_soil_type_dataframe.select_dtypes(np.number).columns]
    interpolation_dataframe.loc[ρ_rel, interpolation_dataframe.select_dtypes(np.number).columns] = np.nan  # clear numerical values

    interpolated_row= (interpolation_dataframe
                    .sort_index()  # sort
                    .interpolate(method="piecewise_polynomial", order=1, axis=0)  # interpolate numerical values
...
def soilmat_interpolated_kwargs(*duncan_selig_soil_type_args, **kwargs):
    if duncan_selig_soil_type_args:
        ...
        soilmat_dict = build_hardening_soil_parameters(*duncan_selig_soil_type_args).squeeze().to_dict()
    else:
        soilmat_dict = {}
...
def soilmat_kwargs(*duncan_selig_soil_type_args, **kwargs):
    if duncan_selig_soil_type_args:
        ...
        soilmat_dict = DUNCAN_SELIG_DATAFRAME.loc[duncan_selig_soil_type_args].squeeze().to_dict()
    else:
        soilmat_dict = {}
    soilmat_dict.update(*kwargs)
    return soilmat_dict
